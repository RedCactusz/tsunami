"""
swe_solver.py — SWE Tsunami Solver
====================================
...
Version : 1.1.0  (refactor: tools spasial dipisah ke spatial_utils.py)
"""
...
try:
    ...
    HAS_NUMBA = True
    print("  [OK] Numba JIT tersedia — akselerasi SWE aktif")
except ImportError:
    HAS_NUMBA = False
    print("  [WARN] Numba tidak tersedia — menggunakan NumPy standar")
...
# (SyntheticBathymetry diimpor dari spatial_utils — lihat import di atas)
...
class LinearSWESolver:
    ...
    def __init__(self,
        ...
        if bathy_grid is not None:
            ...
            if hasattr(self.bathy, 'depth_grid'):
                print('  Membangun grid dari MasterBathymetry...')
                self.H_raw = self.bathy.depth_grid(self.lat_arr, self.lon_arr)
            else:
                raise ValueError('bathy_grid wajib diberikan dari data riil')
...
class TsunamiSimulator:
    ...
    def run(self,
        ...
        print(f"\n[OK] Simulasi selesai — {stats['cpu_time_s']}s CPU, runup Bantul ≈ {runup_m:.2f} m\n")
        return self.result
    ...
    def _make_inundation_geojson(self, solver: LinearSWESolver,
        ...
        if admin_mask and HAS_SHAPELY:
            try:
                ...
                print(f"  [OK] Masker administrasi (Kabupaten Bantul) diaktifkan.")
                print(f"    bbox: lon {admin_bbox[0]:.4f}–{admin_bbox[2]:.4f}, lat {admin_bbox[1]:.4f}–{admin_bbox[3]:.4f}")
            except Exception as e:
                print(f"  [WARN] Gagal menyiapkan admin_mask: {e}")

        if not prepared_mask:
            print("  [WARN] Tidak ada admin_mask — menggunakan fallback bbox Bantul pesisir.")
        ...
        if dem_manager is not None:
            ...
            print(f"  [WARN] DEM tidak tersedia — fallback ke solver.H_raw")
            for j in range(fill_ny):
                lat = float(fill_lat_arr[j])
...
if __name__ == "__main__":
    ...
    print(f"[OK] Hasil disimpan ke {args.out}")
    s = result["statistics"]
